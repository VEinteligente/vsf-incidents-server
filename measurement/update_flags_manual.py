    ...
    for row in rows:
        ...
        if test_key.ignore_data(dns_isp, public_dns):
            ...
            if not queries[0]['failure'] and queries[0]['answers']:
                ...
                control_resolver = []
                dns_result = []
                control_resolver_server = queries[0]['resolver_hostname']
                ...
                for query in queries:
                    ...
                    # eval only flags if is not for the control awnsers
                    if (dns_name != control_resolver_server):
                        ...
                        if query['failure']:
                            ...
                            if (not all(map(lambda v: v in control_resolver, dns_result)) or  ( (control_resolver) and (not dns_result))):
                            ...
                            # !!!To-do: remove before prodcution
                            #else:
                                #logger.info("no flag DNS|"+"_url?_"+"control "+str(control_resolver_server)+": "+str(control_resolver)+" "+str(dns_isp)+" "+str(dns_name)+": "+str(dns_result))
