...
#from django.utils.six import str
...
from .DatatablesForm import DatatablesForm, DESC
...
class DatatablesView(MultipleObjectMixin, View):
    ...
    def get_row(self, row):
        ...
        if isinstance(self.fields, dict):
            return dict([
                (key, str(value).format(**row) if RE_FORMATTED.match(value) else row[value])
                for key, value in self.fields.items()
            ])
        else:
            return [str(field).format(**row) if RE_FORMATTED.match(field)
                    else row[field]
